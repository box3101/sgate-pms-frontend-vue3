      ...
      <div class="icon-menu">
        ...
        <UiModal title="알림" v-model="isNotificationModalOpen" position="right" :size="'small'">
          ...
          <div class="notifications-container">
            ...
            <!-- 알림 예시 -->
            <div v-if="notifications.length === 0" class="no-notifications">알림이 없습니다.</div>
          </div>
        </UiModal>
        ...
        <UiModal v-model="isFullMenuModalOpen" :size="'full'">
          <template #title>
            전체 메뉴
            <span class="menu-description">(메뉴를 클릭하면 해당 메뉴로 이동합니다)</span>
          </template>
                ...
                >
                  ...
                  <div class="submenu-items-grid">
                    <router-link
                      ...
                      @click="isFullMenuModalOpen = false"
                    >
                      <span class="submenu-item-name">{{ submenuItem.name }}</span>
                    </router-link>
                  </div>
                </div>
              </div>
...
<script setup>
  ...
  const currentTabMenu = computed(() => {
    ...
    if (
      ...
      route.path.startsWith('/task/reports/view') ||
      route.path.startsWith('/task/reports/create')
    ) {
      hasLink.value = true
      hasLinkIcon.value = 'icon icon-youtube icon-lg'
