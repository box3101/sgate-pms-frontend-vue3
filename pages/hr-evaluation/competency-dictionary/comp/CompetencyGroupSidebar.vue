  ...
  <div>
    ...
    >
      ...
      <template #header="{ selectAll, isAllSelected }">
        ...
        <th v-for="header in competencyGroupHeaders" :key="header.key">
          {{ header.title }}
        </th>
      </template>
      <template
      ...
      >
        <tr
          ...
          :class="selectedGroupIndex === index ? 'selected-row' : ''"
        >
          <td>
            <UiCheckbox
              size="large"
...
<script setup>
  import { ref } from 'vue'
  const competencyGroupHeaders = ref([
    { key: 'name', title: '템플릿명' },
    { key: 'actions', title: '수정' }
  ])

  const competencyGroupData = ref([
    { id: 1, name: '샘플 템플릿 1' },
    { id: 2, name: '샘플 템플릿 2' },
    { id: 3, name: '샘플 템플릿 3' }
  ])
