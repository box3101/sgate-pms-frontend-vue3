    ...
    >
      ...
      >
        ...
        <div class="detached-modal-actions">
          <button
            class="detached-modal-btn"
            @click="toggleDetachedModalSize(detachedCardItem.id)"
          >
            <Icon
              :name="
                detachedCardItem.isMaximized
                  ? 'mdi:arrow-collapse'
                  : 'mdi:arrow-expand'
              "
              size="16"
          ...
          <button
            class="detached-modal-btn"
            @click="closeDetachedModal(detachedCardItem.id)"
          >
...
function detachCardModal(card) {
  ...
  // 이미 분리된 카드인지 확인
  const existingCardIndex = detachedCards.value.findIndex(
  ...
  const newDetachedCard = {
    ...
    style: {
      left: `${Math.min(
        (windowWidth - 500) / 2 + offset,
        windowWidth - 550
      )}px`,
      top: `${Math.min(
        (windowHeight - 600) / 2 + offset,
        windowHeight - 650
      )}px`,
      width: "500px",
      height: "600px",
...
function handleMouseMove(event) {
  if (isDragging && dragCardId) {
    const cardIndex = detachedCards.value.findIndex(
      (item) => item.id === dragCardId
    );
    if (cardIndex === -1) return;
    ...
    switch (resizeDirection) {
      ...
      case "ne": // 우측 상단
        ...
        card.position = {
          ...
          y:
            resizeStartModalPosition.y - (newHeightNe - resizeStartSize.height),
        };
        card.size = {
          width: Math.max(minWidth, resizeStartSize.width + deltaX),
          height: newHeightNe,
      ...
      case "nw": // 좌측 상단
        ...
        card.position = {
          ...
          y:
            resizeStartModalPosition.y - (newHeightNw - resizeStartSize.height),
        };
        card.size = {
          width: newWidthNw,
          height: newHeightNw,
